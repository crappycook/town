...
type TeeRotateOption struct {
	...
	LevelEnablerFunc LevelEnabler
}

func NewTeeRotateLogger(teeOpts []*TeeRotateOption, zapOpts ...zap.Option) *Logger {
	var cores []zapcore.Core
	cfg := zap.NewProductionConfig()
	cfg.EncoderConfig = getEncoderConfig()

	for _, v := range teeOpts {
		op := v
		lf := zap.LevelEnablerFunc(func(l Level) bool {
			return op.LevelEnablerFunc(l)
		})
		core := zapcore.NewCore(
			zapcore.NewJSONEncoder(cfg.EncoderConfig),
			zapcore.AddSync(op.LogWritter),
			lf,
		)
